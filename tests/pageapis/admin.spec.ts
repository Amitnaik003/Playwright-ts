...
function getSectionUrl(sectionName: string): string {
    ...
    if (slug === 'help') {
        return `${cleanBase}/appcommon/help/mastermain`;
    }
    return `${cleanBase}/appcommon/${slug}`;
}

test(`Admin Pages API Load Test (${tabCount} Parallel Hits x ${repeatCount} Cycles)`, async ({ browser }, testInfo) => {
    test.setTimeout(0);
    ...
    for (let i = 0; i < adminSubSections.length; i++) {
        ...
        try {
            ...
            const menu = setupPage.getByText('Admin', { exact: true }).or(setupPage.getByText(/Admin/i)).first();
            ...
            if (isItemVisible) {
                ...
                if (href && href !== '#' && !href.startsWith('javascript:')) {
                    ...
                    console.log(`[Phase 1 Order ${sectionOrder}/${adminSubSections.length}] UI Menu Item: "${sectionName}" -> Extracted Link: ${fetchedUrl}`);
                } else {
                    ...
                    console.log(`[Phase 1 Order ${sectionOrder}/${adminSubSections.length}] Clicked UI Menu Item: "${sectionName}" -> Dynamically Navigated URL: ${fetchedUrl}`);
                }
            } else {
                fetchedUrl = fallbackUrl;
                console.log(`[Phase 1 Order ${sectionOrder}/${adminSubSections.length}] Menu Item: "${sectionName}" -> Direct Route URL: ${fetchedUrl}`);
            }

            capturedSectionUrls[sectionName] = fetchedUrl;
        } catch (err) {
            capturedSectionUrls[sectionName] = fallbackUrl;
            console.log(`[Phase 1 Order ${sectionOrder}/${adminSubSections.length}] Menu Item: "${sectionName}" -> Fallback URL: ${fallbackUrl}`);
        }
    }
    ...
    console.log('\n================================================================================');
    console.log(` PHASE 2: EXECUTING ${repeatCount} CYCLES & ${adminSubSections.length} ORDERS (${tabCount} PARALLEL HITS PER ORDER)`);
    console.log('================================================================================\n');
    ...
    for (let cycle = 1; cycle <= repeatCount; cycle++) {
        ...
        for (let i = 0; i < adminSubSections.length; i++) {
            ...
            console.log(`\n[Cycle ${cycle}/${repeatCount}] [Order ${sectionOrder}/${adminSubSections.length}: ${sectionName}] Initial API Hit -> Status: ${initialStatusHeader} (${initialStatusText}) | Time: ${initialLatency}ms | URL: ${targetEndpointUrl}`);
            console.log(`[Cycle ${cycle}/${repeatCount}] [Order ${sectionOrder}/${adminSubSections.length}: ${sectionName}] Firing ${tabCount} parallel API hits simultaneously via Promise.all()...`);
            ...
            const hitResults = await Promise.all(
                Array.from({ length: tabCount }).map(async (_, idx): Promise<HitResult> => {
                    ...
                    try {
                        ...
                        console.log(`[Cycle ${cycle}/${repeatCount}] [Order ${sectionOrder}/${adminSubSections.length}] [Tab ${tabIndex}] SAME API TRIGGERED -> ${sectionName} | Status: ${statusDisplay} | TTFB: ${ttfbDisplay} | Payload Size: ${payloadSizeDisplay} | TTI: ${ttiMs}ms | URL: ${requestUrl}`);
                        return { tabIndex, sectionName, success: isOk, apiResponseTimeMs, ttfbMs, payloadSizeBytes, ttiMs, status, requestUrl };
                    } catch (err) {
                        const errorMessage = err instanceof Error ? err.message : String(err);
                        console.warn(`[Cycle ${cycle}/${repeatCount}] [Order ${sectionOrder}/${adminSubSections.length}] [Tab ${tabIndex}] API TRIGGER FAILED -> ${sectionName}:`, errorMessage);
            ...
            if (successfulHits === 0) {
                console.error(`[Cycle ${cycle}/${repeatCount}] [Order ${sectionOrder}/${adminSubSections.length}: ${sectionName}] FAILED: 0/${tabCount} tabs returned valid API response (404 Not Found).`);
            } else {
                console.log(`[Cycle ${cycle}/${repeatCount}] [Order ${sectionOrder}/${adminSubSections.length}: ${sectionName}] Completed: ${successfulHits}/${tabCount} tabs hit SAME API simultaneously via Promise.all().`);
            }
            console.log(`[Order ${sectionOrder}/${adminSubSections.length}: ${sectionName}] TIMING SUMMARY -> Started: ${new Date(orderStartTime).toISOString()} | Ended: ${new Date(orderEndTime).toISOString()} | Total Hit Time: ${orderDurationMs}ms (${(orderDurationMs / 1000).toFixed(2)}s)`);
        ...
        console.log(`\n[Cycle ${cycle}/${repeatCount}] Completed all ${adminSubSections.length} Admin page SAME API hits across all ${tabCount} tabs.`);
    }
...
Sub-Sections Tested (In Order): ${adminSubSections.join(', ')}
...
================================================================================`;
    ...
    attachPerformanceMetrics(testInfo, 'Admin Pages (API)', perfMetrics, {
        tabsCount: tabCount,
        repeatCount,
