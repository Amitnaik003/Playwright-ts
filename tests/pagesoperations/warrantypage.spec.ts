...
test(`Parallel ${tabCount} tabs hitting the EXACT SAME Warranty page API simultaneously at once via Promise.all`, async ({ browser }, testInfo) => {
    test.setTimeout(0);
    ...
    console.log('Creating single shared browser context for Warranty Pages...');
    ...
    for (let cycle = 1; cycle <= repeatCount; cycle++) {
        ...
        for (let i = 0; i < warrantySubSections.length; i++) {
            ...
            console.log(`\n[Cycle ${cycle}/${repeatCount}] [Order ${sectionOrder}/${warrantySubSections.length}: ${sectionName}] Firing ${tabCount} parallel tabs simultaneously...`);
                ...
                activeTabs.map(async ({ tabIndex, page }) => {
                    ...
                    try {
                        ...
                        if (!(await itemLocator.isVisible({ timeout: 500 }).catch(() => false))) {
                            const menu = page.getByText('Warranty', { exact: true }).or(page.getByText(/Warranty/i)).first();
            ...
            console.log(`[Cycle ${cycle}/${repeatCount}] [Order ${sectionOrder}/${warrantySubSections.length}: ${sectionName}] Completed: ${successfulHits}/${tabCount} tabs hit simultaneously.`);
            console.log(`[Order ${sectionOrder}/${warrantySubSections.length}: ${sectionName}] TIMING: ${orderDurationMs}ms (${(orderDurationMs / 1000).toFixed(2)}s)`);
        }
    }
    ...
    attachPerformanceMetrics(testInfo, 'Warranty Pages (Operations)', perfMetrics, {
        tabsCount: tabCount,
        repeatCount,
