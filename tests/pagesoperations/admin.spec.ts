...
test(`Parallel ${tabCount} tabs hitting the EXACT SAME Admin page API simultaneously at once via Promise.all`, async ({ browser }, testInfo) => {
    ...
    for (let cycle = 1; cycle <= repeatCount; cycle++) {
        ...
        for (let i = 0; i < adminSubSections.length; i++) {
            ...
            console.log(`\n[Cycle ${cycle}/${repeatCount}] [Order ${sectionOrder}/${adminSubSections.length}: ${sectionName}] Firing ${tabCount} parallel tabs simultaneously...`);
            ...
            console.log(`[Cycle ${cycle}/${repeatCount}] [Order ${sectionOrder}/${adminSubSections.length}: ${sectionName}] Completed: ${successfulHits}/${tabCount} tabs hit simultaneously.`);
            console.log(`[Order ${sectionOrder}/${adminSubSections.length}: ${sectionName}] TIMING: ${orderDurationMs}ms (${(orderDurationMs / 1000).toFixed(2)}s)`);
        }
    }
    ...
    attachPerformanceMetrics(testInfo, 'Admin Pages (Operations)', perfMetrics, {
        tabsCount: tabCount,
        repeatCount,
