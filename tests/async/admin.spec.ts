...
function getSectionUrl(sectionName: string): string {
    ...
    if (slug === 'help') return `${cleanBase}/appcommon/help/mastermain`;
...
test(`Admin Pages API Load Test (${tabCount} Parallel Hits x ${repeatCount} Cycles)`, async ({ browser }, testInfo) => {
    ...
    console.log('\n================================================================================');
    console.log(' PHASE 1: CLICKING EACH ADMIN SECTION BUTTON IN ORDER TO FETCH RESULTING URL   ');
    console.log('================================================================================');
    ...
    for (let i = 0; i < adminSubSections.length; i++) {
        ...
        try {
            ...
            const adminMenu = setupPage.getByText('Admin', { exact: true }).or(setupPage.getByText(/Admin/i)).first();
            ...
            if (href && href !== '#' && !href.startsWith('javascript:')) {
                ...
                console.log(`[Phase 1 Order ${sectionOrder}/${adminSubSections.length}] Button: "${sectionName}" -> Extracted Link: ${fetchedUrl}`);
            } else {
                ...
                console.log(`[Phase 1 Order ${sectionOrder}/${adminSubSections.length}] Clicked Button: "${sectionName}" -> Dynamically Navigated URL: ${fetchedUrl}`);
            }
            ...
            console.log(`[Phase 1 Order ${sectionOrder}/${adminSubSections.length}] Clicked Button: "${sectionName}" -> Fallback URL: ${fallbackUrl}`);
        }
    }
    ...
    console.log('================================================================================');
    console.log(' PHASE 1 COMPLETE: All Admin Section Button URLs Fetched!                      ');
    console.log('================================================================================\n');
...
================================================================================`;
    ...
    attachPerformanceMetrics(testInfo, 'Admin Pages (Async)', perfMetrics, {
        tabsCount: tabCount,
        repeatCount,
