...
function getSectionUrl(sectionName: string): string {
    ...
    if (slug === 'parts-search' || slug === 'scorecard') return `${cleanBase}/appcommon/report-config`;
    if (slug === 'customer-search' || slug === 'cust-criteria') return `${cleanBase}/appcommon/cust-criteria`;
    if (slug === 'plant-search' || slug === 'report') return `${cleanBase}/appcommon/report`;
    if (slug === 'overall-search' || slug === 'report-configurator-v2') return `${cleanBase}/appcommon/analytics/setup/report-config-v2`;
    if (slug === 'report-criteria' || slug === 'criteria') return `${cleanBase}/appcommon/analytics/setup/report-criteria/display`;
    return `${cleanBase}/appcommon/${slug}`;
}

test(`Analytics Pages API Load Test (${tabCount} Parallel Hits x ${repeatCount} Cycles)`, async ({ browser }, testInfo) => {
    test.setTimeout(0);
    ...
    for (let i = 0; i < analyticsSubSections.length; i++) {
        ...
        try {
            ...
            const menu = setupPage.getByText('Analytics', { exact: true }).or(setupPage.getByText(/Analytics/i)).first();
            if (await menu.isVisible({ timeout: 500 }).catch(() => false)) {
                await menu.click({ force: true }).catch(() => { });
                await setupPage.waitForTimeout(150);
            }

            // Expand nested sub-headers if visible
            for (const subHeader of ['Setup', 'Reports', 'Configuration', 'Report Configurator', 'Search', 'Analytics Setup']) {
                const subLoc = setupPage.getByText(subHeader, { exact: true }).first();
                if (await subLoc.isVisible({ timeout: 300 }).catch(() => false)) {
                    await subLoc.click({ force: true }).catch(() => { });
                    await setupPage.waitForTimeout(100);
                }
            }
            ...
            if (isItemVisible) {
                ...
                if (href && href !== '#' && !href.startsWith('javascript:')) {
                    ...
                    console.log(`[Phase 1 Order ${sectionOrder}/${analyticsSubSections.length}] UI Menu Item: "${sectionName}" -> Extracted Link: ${fetchedUrl}`);
                } else {
                    ...
                    console.log(`[Phase 1 Order ${sectionOrder}/${analyticsSubSections.length}] Clicked UI Menu Item: "${sectionName}" -> Dynamically Navigated URL: ${fetchedUrl}`);
                }
            } else {
                fetchedUrl = fallbackUrl;
                console.log(`[Phase 1 Order ${sectionOrder}/${analyticsSubSections.length}] Menu Item: "${sectionName}" -> Direct Route URL: ${fetchedUrl}`);
            }

            capturedSectionUrls[sectionName] = fetchedUrl;
        } catch (err) {
            capturedSectionUrls[sectionName] = fallbackUrl;
            console.log(`[Phase 1 Order ${sectionOrder}/${analyticsSubSections.length}] Menu Item: "${sectionName}" -> Fallback URL: ${fallbackUrl}`);
        }
    }
    ...
    console.log('\n================================================================================');
    console.log(` PHASE 2: EXECUTING ${repeatCount} CYCLES & ${analyticsSubSections.length} ORDERS (${tabCount} PARALLEL HITS PER ORDER)`);
    console.log('================================================================================\n');
    ...
    for (let cycle = 1; cycle <= repeatCount; cycle++) {
        ...
        for (let i = 0; i < analyticsSubSections.length; i++) {
            ...
            console.log(`\n[Cycle ${cycle}/${repeatCount}] [Order ${sectionOrder}/${analyticsSubSections.length}: ${sectionName}] Initial API Hit -> Status: ${initialStatusHeader} (${initialStatusText}) | Time: ${initialLatency}ms | URL: ${targetEndpointUrl}`);
            console.log(`[Cycle ${cycle}/${repeatCount}] [Order ${sectionOrder}/${analyticsSubSections.length}: ${sectionName}] Firing ${tabCount} parallel API hits simultaneously via Promise.all()...`);
            ...
            const hitResults = await Promise.all(
                Array.from({ length: tabCount }).map(async (_, idx): Promise<HitResult> => {
                    ...
                    try {
                        ...
                        console.log(`[Cycle ${cycle}/${repeatCount}] [Order ${sectionOrder}/${analyticsSubSections.length}] [Tab ${tabIndex}] SAME API TRIGGERED -> ${sectionName} | Status: ${statusDisplay} | TTFB: ${ttfbDisplay} | Payload Size: ${payloadSizeDisplay} | TTI: ${ttiMs}ms | URL: ${requestUrl}`);
                        return { tabIndex, sectionName, success: isOk, apiResponseTimeMs, ttfbMs, payloadSizeBytes, ttiMs, status, requestUrl };
                    } catch (err) {
                        const errorMessage = err instanceof Error ? err.message : String(err);
                        console.warn(`[Cycle ${cycle}/${repeatCount}] [Order ${sectionOrder}/${analyticsSubSections.length}] [Tab ${tabIndex}] API TRIGGER FAILED -> ${sectionName}:`, errorMessage);
            ...
            if (successfulHits === 0) {
                console.error(`[Cycle ${cycle}/${repeatCount}] [Order ${sectionOrder}/${analyticsSubSections.length}: ${sectionName}] FAILED: 0/${tabCount} tabs returned valid API response (404 Not Found).`);
            } else {
                console.log(`[Cycle ${cycle}/${repeatCount}] [Order ${sectionOrder}/${analyticsSubSections.length}: ${sectionName}] Completed: ${successfulHits}/${tabCount} tabs hit SAME API simultaneously via Promise.all().`);
            }
            console.log(`[Order ${sectionOrder}/${analyticsSubSections.length}: ${sectionName}] TIMING SUMMARY -> Started: ${new Date(orderStartTime).toISOString()} | Ended: ${new Date(orderEndTime).toISOString()} | Total Hit Time: ${orderDurationMs}ms (${(orderDurationMs / 1000).toFixed(2)}s)`);
        ...
        console.log(`\n[Cycle ${cycle}/${repeatCount}] Completed all ${analyticsSubSections.length} Analytics page SAME API hits across all ${tabCount} tabs.`);
    }
    ...
    const reportContent = `================================================================================
                    ANALYTICS PAGES PERFORMANCE SUMMARY REPORT                      
================================================================================
Target Module:                  Analytics Pages (${analyticsSubSections.length} Pages)
Total Parallel Calls Fired:     ${tabCount}
Total Repeat Cycles Executed:   ${repeatCount}
Sub-Sections Tested (In Order): ${analyticsSubSections.join(', ')}
...
================================================================================`;
    ...
    testInfo.attachments.push({
        name: 'Analytics Pages Performance Report.txt',
        contentType: 'text/plain',
        body: Buffer.from(reportContent, 'utf-8'),
    });

    attachPerformanceMetrics(testInfo, 'Analytics Pages (API)', perfMetrics, {
        tabsCount: tabCount,
        repeatCount,
