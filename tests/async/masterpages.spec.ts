...
// 10 Master sub-sections in strict sequential order
...
test(`Master Pages API Load Test (${tabCount} Parallel Hits x ${repeatCount} Cycles)`, async ({ browser }, testInfo) => {
    ...
    console.log('\n================================================================================');
    console.log(' PHASE 1: CLICKING EACH MASTER SECTION BUTTON IN ORDER TO FETCH RESULTING URL  ');
    console.log('================================================================================');
    ...
    for (let i = 0; i < masterSubSections.length; i++) {
        ...
        try {
            ...
            const masterMenu = setupPage.getByText('Master', { exact: true }).or(setupPage.getByText(/Master/i)).first();
            if (await masterMenu.isVisible({ timeout: 1000 }).catch(() => false)) {
            ...
            if (href && href !== '#' && !href.startsWith('javascript:')) {
                ...
                console.log(`[Phase 1 Order ${sectionOrder}/${masterSubSections.length}] Button: "${sectionName}" -> Extracted Link: ${fetchedUrl}`);
            } else {
                ...
                console.log(`[Phase 1 Order ${sectionOrder}/${masterSubSections.length}] Clicked Button: "${sectionName}" -> Dynamically Navigated URL: ${fetchedUrl}`);
            }
            ...
            console.log(`[Phase 1 Order ${sectionOrder}/${masterSubSections.length}] Clicked Button: "${sectionName}" -> Fallback URL: ${fallbackUrl}`);
        }
    }
    ...
    console.log('================================================================================');
    console.log(' PHASE 1 COMPLETE: All Master Section Button URLs Fetched!                     ');
    console.log('================================================================================\n');
...
================================================================================`;
    ...
    attachPerformanceMetrics(testInfo, 'Master Pages (Async)', perfMetrics, {
        tabsCount: tabCount,
        repeatCount,
