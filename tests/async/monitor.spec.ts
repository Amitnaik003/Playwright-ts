...
function getSectionUrl(sectionName: string): string {
    ...
    if (slug === 'job-monitor' || slug === 'process-monitor') return `${cleanBase}/appcommon/ProcessMonitor`;
    if (slug === 'email-log' || slug === 'communication-monitor') return `${cleanBase}/appcommon/MailCommunication`;
...
test(`Monitor Pages API Load Test (${tabCount} Parallel Hits x ${repeatCount} Cycles)`, async ({ browser }, testInfo) => {
    ...
    console.log('\n================================================================================');
    console.log(' PHASE 1: CLICKING EACH MONITOR SECTION BUTTON IN ORDER TO FETCH RESULTING URL ');
    console.log('================================================================================');
    ...
    for (let i = 0; i < monitorSubSections.length; i++) {
        ...
        try {
            ...
            if (href && href !== '#' && !href.startsWith('javascript:')) {
                ...
                console.log(`[Phase 1 Order ${sectionOrder}/${monitorSubSections.length}] Button: "${sectionName}" -> Extracted Link: ${fetchedUrl}`);
            } else {
                ...
                console.log(`[Phase 1 Order ${sectionOrder}/${monitorSubSections.length}] Clicked Button: "${sectionName}" -> Dynamically Navigated URL: ${fetchedUrl}`);
            }
            ...
            console.log(`[Phase 1 Order ${sectionOrder}/${monitorSubSections.length}] Clicked Button: "${sectionName}" -> Fallback URL: ${fallbackUrl}`);
        }
    }
    ...
    console.log('================================================================================');
    console.log(' PHASE 1 COMPLETE: All Monitor Section Button URLs Fetched!                    ');
    console.log('================================================================================\n');
...
================================================================================`;
    ...
    attachPerformanceMetrics(testInfo, 'Monitor Pages (Async)', perfMetrics, {
        tabsCount: tabCount,
        repeatCount,
