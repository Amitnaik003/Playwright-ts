...
test(`Parallel ${tabCount} tabs hitting the EXACT SAME Automation page API simultaneously at once via Promise.all`, async ({ browser }, testInfo) => {
    test.setTimeout(0);
    ...
    for (let cycle = 1; cycle <= repeatCount; cycle++) {
        ...
        for (let i = 0; i < automationSubSections.length; i++) {
            ...
            console.log(`\n[Cycle ${cycle}/${repeatCount}] [Order ${sectionOrder}/${automationSubSections.length}: ${sectionName}] Firing ${tabCount} parallel tabs simultaneously...`);
                ...
                activeTabs.map(async ({ tabIndex, page }) => {
                    ...
                    try {
                        ...
                        if (!(await itemLocator.isVisible({ timeout: 500 }).catch(() => false))) {
                            const menu = page.getByText('Automation', { exact: true }).or(page.getByText(/Automation/i)).first();
            ...
            console.log(`[Cycle ${cycle}/${repeatCount}] [Order ${sectionOrder}/${automationSubSections.length}: ${sectionName}] Completed: ${successfulHits}/${tabCount} tabs hit simultaneously.`);
            console.log(`[Order ${sectionOrder}/${automationSubSections.length}: ${sectionName}] TIMING: ${orderDurationMs}ms (${(orderDurationMs / 1000).toFixed(2)}s)`);
        }
    }
    ...
    attachPerformanceMetrics(testInfo, 'Automation Pages (Operations)', perfMetrics, {
        tabsCount: tabCount,
        repeatCount,
