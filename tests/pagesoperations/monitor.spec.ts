...
test(`Parallel ${tabCount} tabs hitting the EXACT SAME Monitor page API simultaneously at once via Promise.all`, async ({ browser }, testInfo) => {
    test.setTimeout(0);
    ...
    console.log('Creating single shared browser context for Monitor Pages...');
    ...
    for (let cycle = 1; cycle <= repeatCount; cycle++) {
        ...
        for (let i = 0; i < monitorSubSections.length; i++) {
            ...
            console.log(`\n[Cycle ${cycle}/${repeatCount}] [Order ${sectionOrder}/${monitorSubSections.length}: ${sectionName}] Firing ${tabCount} parallel tabs simultaneously...`);
                ...
                activeTabs.map(async ({ tabIndex, page }) => {
                    ...
                    try {
                        ...
                        if (!(await itemLocator.isVisible({ timeout: 500 }).catch(() => false))) {
                            const menu = page.getByText('Monitor', { exact: true }).or(page.getByText(/Monitor/i)).first();
            ...
            console.log(`[Cycle ${cycle}/${repeatCount}] [Order ${sectionOrder}/${monitorSubSections.length}: ${sectionName}] Completed: ${successfulHits}/${tabCount} tabs hit simultaneously.`);
            console.log(`[Order ${sectionOrder}/${monitorSubSections.length}: ${sectionName}] TIMING: ${orderDurationMs}ms (${(orderDurationMs / 1000).toFixed(2)}s)`);
        }
    }
    ...
    attachPerformanceMetrics(testInfo, 'Monitor Pages (Operations)', perfMetrics, {
        tabsCount: tabCount,
        repeatCount,
