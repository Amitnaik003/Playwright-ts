...
function getSectionUrl(sectionName: string): string {
    ...
    if (slug === 'process-monitor' || lowerName.includes('process monitor')) return `${cleanBase}/appcommon/ProcessMonitor`;
    if (slug === 'support-report' || lowerName.includes('support report')) return `${cleanBase}/appcommon/SupportReport`;
    if (slug === 'message-monitor' || lowerName.includes('message monitor')) return `${cleanBase}/appcommon/text-message-monitor`;

    return `${cleanBase}/appcommon/${slug}`;
}

test(`Monitor Pages API Load Test (${tabCount} Parallel Hits x ${repeatCount} Cycles)`, async ({ browser }, testInfo) => {
    test.setTimeout(0);
    ...
    for (let i = 0; i < monitorSubSections.length; i++) {
        const sectionName = monitorSubSections[i];
        ...
        try {
            ...
            const menu = setupPage.getByText('Monitor', { exact: true }).or(setupPage.getByText(/Monitor/i)).first();
            ...
            if (isItemVisible) {
                ...
                if (href && href !== '#' && !href.startsWith('javascript:')) {
                    ...
                    console.log(`[Phase 1 Order ${sectionOrder}/${monitorSubSections.length}] UI Menu Item: "${sectionName}" -> Extracted Link: ${fetchedUrl}`);
                } else {
                    ...
                    console.log(`[Phase 1 Order ${sectionOrder}/${monitorSubSections.length}] Clicked UI Menu Item: "${sectionName}" -> Dynamically Navigated URL: ${fetchedUrl}`);
                }
            } else {
                fetchedUrl = fallbackUrl;
                console.log(`[Phase 1 Order ${sectionOrder}/${monitorSubSections.length}] Menu Item: "${sectionName}" -> Direct Route URL: ${fetchedUrl}`);
            }
            ...
            console.log(`[Phase 1 Order ${sectionOrder}/${monitorSubSections.length}] Menu Item: "${sectionName}" -> Fallback URL: ${fallbackUrl}`);
        }
    }
    ...
    console.log('\n================================================================================');
    console.log(` PHASE 2: EXECUTING ${repeatCount} CYCLES & ${monitorSubSections.length} ORDERS (${tabCount} PARALLEL HITS PER ORDER)`);
    console.log('================================================================================\n');
    ...
    for (let cycle = 1; cycle <= repeatCount; cycle++) {
        ...
        for (let i = 0; i < monitorSubSections.length; i++) {
            ...
            console.log(`\n[Cycle ${cycle}/${repeatCount}] [Order ${sectionOrder}/${monitorSubSections.length}: ${sectionName}] Initial API Hit -> Status: ${initialStatusHeader} (${initialStatusText}) | Time: ${initialLatency}ms | URL: ${targetEndpointUrl}`);
            console.log(`[Cycle ${cycle}/${repeatCount}] [Order ${sectionOrder}/${monitorSubSections.length}: ${sectionName}] Firing ${tabCount} parallel API hits simultaneously via Promise.all()...`);
            ...
            const hitResults = await Promise.all(
                Array.from({ length: tabCount }).map(async (_, idx): Promise<HitResult> => {
                    ...
                    try {
                        ...
                        console.log(`[Cycle ${cycle}/${repeatCount}] [Order ${sectionOrder}/${monitorSubSections.length}] [Tab ${tabIndex}] SAME API TRIGGERED -> ${sectionName} | Status: ${statusDisplay} | TTFB: ${ttfbDisplay} | Payload Size: ${payloadSizeDisplay} | TTI: ${ttiMs}ms | URL: ${requestUrl}`);
                        return { tabIndex, sectionName, success: isOk, apiResponseTimeMs, ttfbMs, payloadSizeBytes, ttiMs, status, requestUrl };
                    } catch (err) {
                        const errorMessage = err instanceof Error ? err.message : String(err);
                        console.warn(`[Cycle ${cycle}/${repeatCount}] [Order ${sectionOrder}/${monitorSubSections.length}] [Tab ${tabIndex}] API TRIGGER FAILED -> ${sectionName}:`, errorMessage);
            ...
            if (successfulHits === 0) {
                console.error(`[Cycle ${cycle}/${repeatCount}] [Order ${sectionOrder}/${monitorSubSections.length}: ${sectionName}] FAILED: 0/${tabCount} tabs returned valid API response (404 Not Found).`);
            } else {
                console.log(`[Cycle ${cycle}/${repeatCount}] [Order ${sectionOrder}/${monitorSubSections.length}: ${sectionName}] Completed: ${successfulHits}/${tabCount} tabs hit SAME API simultaneously via Promise.all().`);
            }
            console.log(`[Order ${sectionOrder}/${monitorSubSections.length}: ${sectionName}] TIMING SUMMARY -> Started: ${new Date(orderStartTime).toISOString()} | Ended: ${new Date(orderEndTime).toISOString()} | Total Hit Time: ${orderDurationMs}ms (${(orderDurationMs / 1000).toFixed(2)}s)`);
        ...
        console.log(`\n[Cycle ${cycle}/${repeatCount}] Completed all ${monitorSubSections.length} Monitor page SAME API hits across all ${tabCount} tabs.`);
    }
    ...
    const reportContent = `================================================================================
                    MONITOR PAGES PERFORMANCE SUMMARY REPORT                      
================================================================================
Target Module:                  Monitor Pages (${monitorSubSections.length} Pages)
Total Parallel Calls Fired:     ${tabCount}
Total Repeat Cycles Executed:   ${repeatCount}
Sub-Sections Tested (In Order): ${monitorSubSections.join(', ')}
...
================================================================================`;
    ...
    testInfo.attachments.push({
        name: 'Monitor Pages Performance Report.txt',
        contentType: 'text/plain',
        body: Buffer.from(reportContent, 'utf-8'),
    });

    attachPerformanceMetrics(testInfo, 'Monitor Pages (API)', perfMetrics, {
        tabsCount: tabCount,
        repeatCount,
