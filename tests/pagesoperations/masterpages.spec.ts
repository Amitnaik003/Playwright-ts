...
test(`Parallel ${tabCount} tabs hitting the EXACT SAME Master page API simultaneously at once via Promise.all`, async ({ browser }, testInfo) => {
    ...
    for (let cycle = 1; cycle <= repeatCount; cycle++) {
        ...
        for (let i = 0; i < masterSubSections.length; i++) {
            ...
            console.log(`\n[Cycle ${cycle}/${repeatCount}] [Order ${sectionOrder}/${masterSubSections.length}: ${sectionName}] Firing ${tabCount} parallel tabs simultaneously...`);
            ...
            console.log(`[Cycle ${cycle}/${repeatCount}] [Order ${sectionOrder}/${masterSubSections.length}: ${sectionName}] Completed: ${successfulHits}/${tabCount} tabs hit simultaneously.`);
            console.log(`[Order ${sectionOrder}/${masterSubSections.length}: ${sectionName}] TIMING: ${orderDurationMs}ms (${(orderDurationMs / 1000).toFixed(2)}s)`);
        }
    }
    ...
    attachPerformanceMetrics(testInfo, 'Master Pages (Operations)', perfMetrics, {
        tabsCount: tabCount,
        repeatCount,
