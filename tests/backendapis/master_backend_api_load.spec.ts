...
test(`Master Pages Backend REST API Load Test - Real Azure App Service Spikes (${hitCount} Parallel Hits x ${repeatCount} Cycles)`, async ({ browser }, testInfo) => {
    ...
    console.log('================================================================================');
    console.log(' PHASE 1: STRICT UI LOGIN & INTERCEPTING MASTER PAGES BACKEND REST API ENDPOINTS');
    console.log('================================================================================');
    ...
    for (let cycle = 1; cycle <= repeatCount; cycle++) {
        console.log(`==================================================`);
        console.log(` Starting Master Synchronized Cycle [${cycle}/${repeatCount}]`);
        console.log(`==================================================`);
...
================================================================================`;
    ...
    if (perfMetrics) {
        attachPerformanceMetrics(testInfo, 'Master Pages Backend REST API Load Test', perfMetrics, {
            tabsCount: hitCount,
            repeatCount,
    ...
    const jsonLogPath = path.join(logsDir, `failed_master_backend_api_hits.json`);
    const txtLogPath = path.join(logsDir, `failed_master_backend_api_hits.log`);
    ...
    let txtLogContent = `================================================================================\n`;
    txtLogContent += ` FAILED MASTER BACKEND REST API HITS LOG - ${new Date().toISOString()}\n`;
