...
function getSectionUrl(sectionName: string): string {
    ...
    if (slug === 'job-triggers' || slug === 'plant-customer-cross-ref') return `${cleanBase}/appcommon/plantcustomerref`;
    if (slug === 'alert-rules' || slug === 'program-mapping') return `${cleanBase}/appcommon/program/bi`;
    if (slug === 'workflows' || slug === 'program-variant') return `${cleanBase}/appcommon/program-variant`;
    if (slug === 'logs' || slug === 'task') return `${cleanBase}/appcommon/task/bi`;
    if (slug === 'schedule-job') return `${cleanBase}/appcommon/job`;
...
test(`Automation Pages API Load Test (${tabCount} Parallel Hits x ${repeatCount} Cycles)`, async ({ browser }, testInfo) => {
    ...
    console.log('\n================================================================================');
    console.log(' PHASE 1: CLICKING EACH AUTOMATION SECTION BUTTON IN ORDER TO FETCH RESULTING URL');
    console.log('================================================================================');
    ...
    for (let i = 0; i < automationSubSections.length; i++) {
        ...
        try {
            ...
            if (href && href !== '#' && !href.startsWith('javascript:')) {
                ...
                console.log(`[Phase 1 Order ${sectionOrder}/${automationSubSections.length}] Button: "${sectionName}" -> Extracted Link: ${fetchedUrl}`);
            } else {
                ...
                console.log(`[Phase 1 Order ${sectionOrder}/${automationSubSections.length}] Clicked Button: "${sectionName}" -> Dynamically Navigated URL: ${fetchedUrl}`);
            }
            ...
            console.log(`[Phase 1 Order ${sectionOrder}/${automationSubSections.length}] Clicked Button: "${sectionName}" -> Fallback URL: ${fallbackUrl}`);
        }
    }
    ...
    console.log('================================================================================');
    console.log(' PHASE 1 COMPLETE: All Automation Section Button URLs Fetched!                  ');
    console.log('================================================================================\n');
...
================================================================================`;
    ...
    attachPerformanceMetrics(testInfo, 'Automation Pages (Async)', perfMetrics, {
        tabsCount: tabCount,
        repeatCount,
