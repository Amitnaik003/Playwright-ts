...
function getSectionUrl(sectionName: string): string {
    ...
    if (slug === 'plant-customer-cross-ref' || slug === 'job-triggers' || lowerName.includes('plant customer')) return `${cleanBase}/appcommon/plantcustomerref`;
    if (slug === 'program-mapping' || lowerName.includes('program mapping')) return `${cleanBase}/appcommon/program/bi`;
    if (slug === 'program-variant' || lowerName.includes('program variant')) return `${cleanBase}/appcommon/program-variant`;
    if (slug === 'task' || lowerName === 'task') return `${cleanBase}/appcommon/task/bi`;
    if (slug === 'scheduler' || lowerName === 'scheduler' || slug === 'job') return `${cleanBase}/appcommon/job`;
    if (slug === 'mail' || lowerName === 'mail') return `${cleanBase}/appcommon/adapters/mail`;
    if (slug === 'ftp-sftp' || slug === 'ftp/sftp' || lowerName.includes('ftp')) return `${cleanBase}/appcommon/adapters/ftp/server/connections`;
    if (slug === 'logical-system' || lowerName.includes('logical system')) return `${cleanBase}/appcommon/logical_system`;
    if (slug === 'internal-customer-cross-ref' || lowerName.includes('internal customer')) return `${cleanBase}/appcommon/customerref`;
    if (slug === 'screen-configuration' || lowerName.includes('screen configuration')) return `${cleanBase}/appcommon/screen-config/display`;

    return `${cleanBase}/appcommon/${slug}`;
}

test(`Automation Pages API Load Test (${tabCount} Parallel Hits x ${repeatCount} Cycles)`, async ({ browser }, testInfo) => {
    test.setTimeout(0);
    ...
    for (let i = 0; i < automationSubSections.length; i++) {
        ...
        try {
            ...
            const menu = setupPage.getByText('Automation', { exact: true }).or(setupPage.getByText(/Automation/i)).first();
            ...
            if (isItemVisible) {
                ...
                if (href && href !== '#' && !href.startsWith('javascript:')) {
                    ...
                    console.log(`[Phase 1 Order ${sectionOrder}/${automationSubSections.length}] UI Menu Item: "${sectionName}" -> Extracted Link: ${fetchedUrl}`);
                } else {
                    ...
                    console.log(`[Phase 1 Order ${sectionOrder}/${automationSubSections.length}] Clicked UI Menu Item: "${sectionName}" -> Dynamically Navigated URL: ${fetchedUrl}`);
                }
            } else {
                fetchedUrl = fallbackUrl;
                console.log(`[Phase 1 Order ${sectionOrder}/${automationSubSections.length}] Menu Item: "${sectionName}" -> Direct Route URL: ${fetchedUrl}`);
            }
            ...
            console.log(`[Phase 1 Order ${sectionOrder}/${automationSubSections.length}] Menu Item: "${sectionName}" -> Fallback URL: ${fallbackUrl}`);
        }
    }
    ...
    console.log('\n================================================================================');
    console.log(` PHASE 2: EXECUTING ${repeatCount} CYCLES & ${automationSubSections.length} ORDERS (${tabCount} PARALLEL HITS PER ORDER)`);
    console.log('================================================================================\n');
    ...
    for (let cycle = 1; cycle <= repeatCount; cycle++) {
        ...
        for (let i = 0; i < automationSubSections.length; i++) {
            ...
            console.log(`\n[Cycle ${cycle}/${repeatCount}] [Order ${sectionOrder}/${automationSubSections.length}: ${sectionName}] Initial API Hit -> Status: ${initialStatusHeader} (${initialStatusText}) | Time: ${initialLatency}ms | URL: ${targetEndpointUrl}`);
            console.log(`[Cycle ${cycle}/${repeatCount}] [Order ${sectionOrder}/${automationSubSections.length}: ${sectionName}] Firing ${tabCount} parallel API hits simultaneously via Promise.all()...`);
            ...
            const hitResults = await Promise.all(
                Array.from({ length: tabCount }).map(async (_, idx): Promise<HitResult> => {
                    ...
                    try {
                        ...
                        console.log(`[Cycle ${cycle}/${repeatCount}] [Order ${sectionOrder}/${automationSubSections.length}] [Tab ${tabIndex}] SAME API TRIGGERED -> ${sectionName} | Status: ${statusDisplay} | TTFB: ${ttfbDisplay} | Payload Size: ${payloadSizeDisplay} | TTI: ${ttiMs}ms | URL: ${requestUrl}`);
                        return { tabIndex, sectionName, success: isOk, apiResponseTimeMs, ttfbMs, payloadSizeBytes, ttiMs, status, requestUrl };
                    } catch (err) {
                        const errorMessage = err instanceof Error ? err.message : String(err);
                        console.warn(`[Cycle ${cycle}/${repeatCount}] [Order ${sectionOrder}/${automationSubSections.length}] [Tab ${tabIndex}] API TRIGGER FAILED -> ${sectionName}:`, errorMessage);
            ...
            if (successfulHits === 0) {
                console.error(`[Cycle ${cycle}/${repeatCount}] [Order ${sectionOrder}/${automationSubSections.length}: ${sectionName}] FAILED: 0/${tabCount} tabs returned valid API response (404 Not Found).`);
            } else {
                console.log(`[Cycle ${cycle}/${repeatCount}] [Order ${sectionOrder}/${automationSubSections.length}: ${sectionName}] Completed: ${successfulHits}/${tabCount} tabs hit SAME API simultaneously via Promise.all().`);
            }
            console.log(`[Order ${sectionOrder}/${automationSubSections.length}: ${sectionName}] TIMING SUMMARY -> Started: ${new Date(orderStartTime).toISOString()} | Ended: ${new Date(orderEndTime).toISOString()} | Total Hit Time: ${orderDurationMs}ms (${(orderDurationMs / 1000).toFixed(2)}s)`);
        ...
        console.log(`\n[Cycle ${cycle}/${repeatCount}] Completed all ${automationSubSections.length} Automation page SAME API hits across all ${tabCount} tabs.`);
    }
...
Sub-Sections Tested (In Order): ${automationSubSections.join(', ')}
...
================================================================================`;
    ...
    attachPerformanceMetrics(testInfo, 'Automation Pages (API)', perfMetrics, {
        tabsCount: tabCount,
        repeatCount,
