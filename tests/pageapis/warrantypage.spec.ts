...
/**
 * Builds accurate default routing URLs for Warranty sub-sections
 */
function getSectionUrl(sectionName: string): string {
    const cleanBase = targetUrl.replace(/\/$/, '');
    const slug = sectionName.toLowerCase().replace(/\s+/g, '-');

    const warrantyRoutes: Record<string, string> = {
        'common-data-view': `${cleanBase}/appcommon/warranty/data-view`,
        'part-return-analysis': `${cleanBase}/appcommon/warranty/part-return-analysis`,
        'final-conclusion': `${cleanBase}/appcommon/warranty/master-data/part-return/conclusion/final-conclusion/display`,
        'failure-mode': `${cleanBase}/appcommon/warranty/master-data/part-return/conclusion/failure-mode/display`,
        'rootcause-type': `${cleanBase}/appcommon/warranty/master-data/part-return/conclusion/root-cause/display`,
        'test-type': `${cleanBase}/appcommon/warranty/master-data/part-return/conclusion/test-type/display`,
        'lookup-table': `${cleanBase}/appcommon/warranty/master-data/data-processing/lookup-table`
    };

    return warrantyRoutes[slug] || `${cleanBase}/appcommon/warranty/${slug}`;
}
...
test(`Warranty Pages API Load Test (${tabCount} Parallel Hits x ${repeatCount} Cycles)`, async ({ browser }, testInfo) => {
    ...
    for (let i = 0; i < warrantySubSections.length; i++) {
        ...
        try {
            // 1. Fill search input box if available in UI (with short 500ms timeout)
            const searchInput = setupPage.locator('input[placeholder*="search" i], input[type="search"], .sidebar input[placeholder*="search" i], nav input[placeholder*="search" i]').first();
            ...
            // 2. Open main Warranty menu header
            ...
            // 3. Expand nested sub-headers if visible
            for (const subHeader of ['Master Data', 'Part Return', 'Conclusion', 'Data Processing', 'Part Return Conclusion']) {
            ...
            // 4. Locate target section item with 1-second timeout guard
            ...
            if (isItemVisible) {
                // 5. Extract link href / routerlink with fast 1s timeout guard
                ...
                if (href && href !== '#' && !href.startsWith('javascript:')) {
                    ...
                    console.log(`[Phase 1 Order ${sectionOrder}/${warrantySubSections.length}] UI Menu Item: "${sectionName}" -> Extracted Link: ${fetchedUrl}`);
                } else {
                    ...
                    console.log(`[Phase 1 Order ${sectionOrder}/${warrantySubSections.length}] Clicked UI Menu Item: "${sectionName}" -> Dynamically Navigated URL: ${fetchedUrl}`);
                }
            } else {
                fetchedUrl = fallbackUrl;
                console.log(`[Phase 1 Order ${sectionOrder}/${warrantySubSections.length}] Menu Item: "${sectionName}" -> Direct Route URL: ${fetchedUrl}`);
            }

            capturedSectionUrls[sectionName] = fetchedUrl;
        } catch (err) {
            capturedSectionUrls[sectionName] = fallbackUrl;
            console.log(`[Phase 1 Order ${sectionOrder}/${warrantySubSections.length}] Menu Item: "${sectionName}" -> Fallback URL: ${fallbackUrl}`);
        }
    }
    ...
    for (let cycle = 1; cycle <= repeatCount; cycle++) {
        ...
        for (let i = 0; i < warrantySubSections.length; i++) {
            ...
            if (successfulHits === 0) {
                console.error(`[Cycle ${cycle}/${repeatCount}] [Order ${sectionOrder}/${warrantySubSections.length}: ${sectionName}] FAILED: 0/${tabCount} tabs returned valid API response (404 Not Found).`);
            } else {
...
================================================================================`;
    ...
    if (typeof attachPerformanceMetrics === 'function') {
        attachPerformanceMetrics(testInfo, 'Warranty Pages (API)', perfMetrics, {
            tabsCount: tabCount,
            repeatCount,
